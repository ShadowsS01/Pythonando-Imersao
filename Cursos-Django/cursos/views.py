...
def aula(request, id):
    if request.session.get("usuario"):
        ...
        request_usuario = request.session.get("usuario")
        usuario_avaliou = NotasAulas.objects.filter(aula_id=id).filter(
            usuario_id=request_usuario
        )
...
def comentarios(request):
    ...
    comentario_instancia = Comentarios(
        usuario_id=usuario_id, comentario=comentario, aula_id=aula_id
    )
...
def processa_avaliacao(request):
    if request.session.get("usuario"):
        ...
        usuario_avaliou = NotasAulas.objects.filter(aula_id=aula_id).filter(
            usuario_id=usuario_id
        )

        if not usuario_avaliou:
            nota_aulas = NotasAulas(
                aula_id=aula_id,
                nota=avaliacao,
                usuario_id=usuario_id,
            )
