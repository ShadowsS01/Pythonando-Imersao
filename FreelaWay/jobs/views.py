...
def encontrar_jobs(request):
    if request.method == "GET":
        ...
        if preco_minimo or preco_maximo or prazo_minimo or prazo_maximo or categoria:
            ...
            jobs = (
                Jobs.objects.filter(preco__gte=preco_minimo)
    ...
    if request.method == "GET":
        ...
        if len(username.strip()) == 0:
            messages.add_message(
                request, constants.ERROR, "Username não pode ser vazio!"
            )
            return redirect("/jobs/perfil")
        elif len(email.strip()) == 0:
            messages.add_message(request, constants.ERROR, "Email não pode ser vazio!")
            return redirect("/jobs/perfil")
        ...
        if usuario.exists():
            messages.add_message(
                request, constants.ERROR, "Já existe um usuário com esse E-mail"
            )
            return redirect("/jobs/perfil")
