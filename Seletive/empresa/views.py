...
def nova_empresa(request):
    if request.method == "GET":
        ...
        if (
            ...
            or len(cidade.strip()) == 0
            or len(endereco.strip()) == 0
            or len(nicho.strip()) == 0
            or len(caracteristicas.strip()) == 0
            or (not logo)
        ):
        ...
        if logo.size > 100_000_000:
            messages.add_message(
                request, constants.ERROR, "A logo da empresa deve ter menos de 10MB"
            )
            return redirect("nova_empresa")
...
def empresas(request):
    ...
    if not empresas:
        messages.add_message(
            request, constants.INFO, "Não tem empresas com esses filtros!"
        )
